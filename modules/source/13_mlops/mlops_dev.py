...
def test_retraining_trigger():
    ...
    assert trigger.accuracy_threshold == 0.85
    assert trigger.drift_threshold == 2
...
def test_mlops_pipeline():
    ...
    health = pipeline.check_system_health(
        new_data=np.random.normal(3, 1, (100, 5)),  # Drifted data
        current_accuracy=0.80  # Low accuracy
    )
    assert health["pipeline_active"] == True
    assert health["drift_detected"] == True
    assert health["retraining_triggered"] == True
