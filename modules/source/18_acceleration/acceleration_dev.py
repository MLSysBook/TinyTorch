...
Before we learn acceleration techniques, let's profile a CNN to understand 
its bottleneck. We'll see it's compute-bound and not using hardware efficiently.
"""

# %%
# Profile CNN to discover compute bottleneck
...
# Create CNN layers of increasing size
print("🔬 Profiling CNN Layers (Compute Efficiency):\n")
print("   Channels  |  FLOPs    |  Latency  |  GFLOP/s  |  Efficiency  |  Bottleneck")
print("   ----------|-----------|-----------|-----------|--------------|-------------")
...
for in_ch, out_ch in configs:
    ...
    input_data = Tensor(np.random.randn(16, in_ch, 32, 32))  # batch of feature maps
    
    # Profile the convolution
    profile = profiler.profile_forward_pass(conv, input_data)
    
    flops_millions = profile['flops'] / 1e6
    latency_ms = profile['latency_ms']
    gflops_per_sec = profile['gflops_per_second']
    efficiency = profile['computational_efficiency']
    bottleneck = profile['bottleneck']
    
    print(f"   {in_ch:2d}→{out_ch:3d}   |  {flops_millions:6.1f}M |  {latency_ms:6.2f}ms |  {gflops_per_sec:6.2f}  |  {efficiency*100:5.1f}%      |  {bottleneck}")

print("\n💡 Key Observations:")
print("   • Convolutions are COMPUTE-BOUND (lots of FLOPs)")
print("   • Low efficiency: only using 10-30% of hardware potential")
print("   • Bottleneck is computation, not memory transfer")
print("   • As problem size grows, efficiency stays low")

print("\n🎯 The Problem:")
print("   Why aren't we using the hardware efficiently?")
print("   • Operations run sequentially (no parallelism)")
print("   • Memory access patterns not cache-friendly")
print("   • Not leveraging SIMD (Single Instruction, Multiple Data)")
print("   • Missing opportunities for operation fusion")

print("\n✨ The Solution:")
print("   Acceleration through:")
print("   • Vectorization: Process multiple data points simultaneously")
print("   • Kernel fusion: Combine operations to reduce memory transfers")
print("   • Cache optimization: Arrange data for better locality")
print("   • Expected improvement: 2-5× speedup with same accuracy!\n")
