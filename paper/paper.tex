...
We present the design and implementation of three pedagogical patterns for teaching ML as systems engineering. \textbf{Progressive disclosure} activates dormant tensor features across modules through monkey-patching, modeling how frameworks evolve from separate abstractions to unified interfaces. \textbf{Systems-first curriculum} embeds memory profiling and complexity analysis from the start rather than treating them as advanced topics. \textbf{Historical milestone validation} recreates nearly 70 years of ML breakthroughs (1958 Perceptron through modern transformers) using exclusively student-implemented code to validate correctness. These patterns are grounded in established learning theory but represent testable design hypotheses whose learning outcomes require empirical validation.
...
% Feature layers - stacked above timeline with increased spacing
% Layer 1: Basic Tensor (always present)
\node[active] at (1.5, 1.5) {\texttt{.data}};
\node[active] at (3.5, 1.5) {\texttt{.shape}};
\draw[orange!80, thick] (0.3, 1.5) -- (13.7, 1.5);
\node[left, font=\tiny] at (0.2, 1.5) {Core};

% Layer 2: Gradient features - increased vertical spacing
\node[dormant] at (1.5, 2.7) {\texttt{.requires\_grad}};
\draw[gray!60, thick, dashed] (0.3, 2.7) -- (5.5, 2.7);
\node[active] at (7.5, 2.7) {\texttt{.requires\_grad}};
\draw[orange!80, thick] (6.3, 2.7) -- (13.7, 2.7);
\node[left, font=\tiny] at (0.2, 2.7) {Gradient};

\node[dormant] at (3.0, 3.6) {\texttt{.grad}};
\draw[gray!60, thick, dashed] (0.3, 3.6) -- (5.5, 3.6);
\node[active] at (7.5, 3.6) {\texttt{.grad}};
\draw[orange!80, thick] (6.3, 3.6) -- (13.7, 3.6);

\node[dormant] at (2.0, 4.5) {\texttt{.backward()}};
\draw[gray!60, thick, dashed] (0.3, 4.5) -- (5.5, 4.5);
\node[active] at (7.5, 4.5) {\texttt{.backward()}};
\draw[orange!80, thick] (6.3, 4.5) -- (13.7, 4.5);

% Activation marker at Module 05
\node[draw, fill=yellow!30, circle, font=\tiny\bfseries, minimum size=0.8cm] at (6, 5.5) {ACTIVATE};
\draw[thick, ->] (6, 5.2) -- (6, 4.7);

% Annotations - adjusted positions to avoid overlap
\node[align=center, font=\tiny] at (3, 6.5) {
    \textbf{Modules 01-04:}\\
    Features visible but dormant\\
    \texttt{.backward()} is no-op
};

\node[align=center, font=\tiny] at (10, 6.5) {
    \textbf{Modules 05-20:}\\
...
% Legend
\node[dormant, minimum width=1.0cm, minimum height=0.4cm] at (2.5, -1.5) {Dormant};
